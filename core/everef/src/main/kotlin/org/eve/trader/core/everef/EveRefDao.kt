...
object EveRefDao {
    ...
    fun getDownloadedDates(): List<String> = DatabaseManager.transaction {
    ...
    fun markDownloaded(date: String, fileName: String, size: Long) = DatabaseManager.transaction {
        prepareStatement(
    ...
    fun deleteBeforeDate(date: String) = DatabaseManager.transaction {
    ...
    fun getDownloadCount(): Int = DatabaseManager.transaction {
        createStatement().executeQuery("SELECT COUNT(*) FROM everef_downloads").use { rs ->
            if (rs.next()) rs.getInt(1) else 0
        }
    }

    fun getEarliestDate(): String? = DatabaseManager.transaction {
        createStatement().executeQuery("SELECT MIN(date) FROM everef_downloads").use { rs ->
            if (rs.next()) rs.getString(1) else null
        }
    }

    fun getLatestDate(): String? = DatabaseManager.transaction {
