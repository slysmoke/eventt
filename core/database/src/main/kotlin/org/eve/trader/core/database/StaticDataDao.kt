...
object StaticDataDao {
    ...
    fun insertType(type: StaticTypeModel) {
        DatabaseManager.transaction {
            prepareStatement(
                """
                INSERT OR REPLACE INTO static_types (type_id, name, group_id, category_id, volume, packaged_volume, portion_size, description, icon_id, published)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """.trimIndent()
            ).use { stmt ->
    ...
    fun searchTypes(query: String, limit: Int = 50): List<StaticTypeModel> {
        return DatabaseManager.transaction {
            prepareStatement("SELECT * FROM static_types WHERE published = 1 AND name LIKE ? ORDER BY name LIMIT ?").use { stmt ->
    ...
    fun getTypesByGroup(groupId: Int): List<StaticTypeModel> {
        return DatabaseManager.transaction {
            prepareStatement("SELECT * FROM static_types WHERE group_id = ? AND published = 1 ORDER BY name").use { stmt ->
    ...
    fun bulkInsertStations(stations: List<StaticStationModel>) {
        stations.forEach { insertStation(it) }
    }
    ...
    fun bulkInsertRegions(regions: List<StaticRegionModel>) {
        regions.forEach { insertRegion(it) }
    }
    ...
    fun bulkInsertSystems(systems: List<StaticSystemModel>) {
        systems.forEach { insertSystem(it) }
    }
