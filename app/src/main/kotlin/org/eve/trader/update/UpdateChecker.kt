...
object UpdateChecker {
    ...
    fun downloadAndInstall(
        ...
        try {
            val updateDir = File(System.getProperty("user.home"), ".eve-trader/update")
                .also { it.mkdirs() }
        ...
        val script = buildString {
            ...
            if (newAppDir != null && installDir != null) {
                ...
                val exe = if (platform == "macos")
                    "${installDir.absolutePath}/Contents/MacOS/eve-trader"
                else
                    "${installDir.absolutePath}/bin/eve-trader"
                appendLine("exec '$exe'")
            } else {
    ...
    private fun restartWindows(newAppDir: File?, installDir: File?, updateDir: File) {
        val script = buildString {
            ...
            if (newAppDir != null && installDir != null) {
                ...
                appendLine("start \"\" \"${installDir.absolutePath}\\bin\\eve-trader.exe\"")
            } else {
                appendLine("echo Update downloaded to: ${updateDir.absolutePath}\\extracted")
                appendLine("pause")
            }
