...
Для локального теста обеих сторон сделки `./scripts/run-p2p-test.sh` поднимает второй, изолированный экземпляр приложения (своя БД и Nostr-идентичность, с тестовым персонажем) — публикуешь ордер в одном окне и отвечаешь из другого.
